...
- Suppose $F_X^{-1}(\cdot)$ is the inverse cdf
  - i.e., $F_{X}^{-1}(u) = \Big\{x \ \text{ s.t. } \  P(X\leqx) = u\Big\}.$
...
-Thus (and remembering that $F_Y^{-1}$ is monotonic increasing):
  ...
  - $P\Big(F_Y^{-1}(U)\leqy\Big)=P\Big(U\leqF_Y(y)\Big) = F_Y(y).$
  - e.g., $P\Big(F_Y^{-1}(U)\leq-1.96\Big) = P(U\leq0.025) = 0.025 = P(Y\leq-1.96)$
  - e.g., $P\Big(F_Y^{-1}(U)\leq0\Big) = P(U\leq0.5) = 0.5 = P(Y\leq0)$
...
- $F_k(\cdot)$ is a cdf for $X_k$ given $X_1,\dots, X_{k-1}.$
  - i.e., $F_k(x_k \mid x_{k-1}, \dots, x_1) = P(X_k\leqx_k \mid x_{k-1}, \dots, x_1).$
- $F_k^{-1}(\cdot)$ is the inverse cdf, i.e.,
   $$\begin{align}& F_{k}^{-1}(u \mid x_{k-1}, \dots, x_1) \nonumber \\  =& \Big\{x_k \ \text{ s.t. } \  P(X_k\leqx_k \mid x_{k-1}, \dots, x_1) = u\Big\}. \nonumber \end{align}$$
...
-  $Z_1=\begin{cases} 1 & \text{ if } U_1\leq0.5 \\ 0 & \text{ if } U_1\geq 0.5 \end{cases}$
- $Z_2=\begin{cases} 1 & \text{ if } U_2\leq0.25 \text{ and } Z_1=1 \\ 0 & \text{ if } U_2 \geq 0.25 \text{ and } Z_1=1 \\ 1 & \text{ if } U_2\leq0.75 \text{ and } Z_1=0   \\ 0 & \text{ if } U_2\geq 0.75 \text{ and } Z_1=0 \end{cases}.$
